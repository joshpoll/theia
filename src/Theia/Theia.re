...
and evalCtx = {
  ops: list(ReasonReact.reactElement),
  args: list(theiaIR),
  holePos: int,
};
...
let rec theiaIRDebugPrint = k =>
  ...
  | Atom(s) => "Atom(" ++ /* s ++ */ ")"
  | Apply2(ss, args) =>
    "Apply2("  /* ++ prettyList(List.map((s) => "\"" ++ s ++ "\"", ss)) */
    ++ ", "
    ++ prettyList(List.map(theiaIRDebugPrint, args))
    ++ ")"
  | Value2(ss, args) =>
    "Value2("
    ++ prettyList(List.map(s => "\"" ++ s ++ "\"", ss))
    ++ ", "
    ++ prettyList(List.map(theiaIRDebugPrint, args))
    ++ ")"
  ...
  | Cell2(s, args) =>
    "Cell2(" ++ s ++ ", " ++ prettyList(List.map(theiaIRDebugPrint, args)) ++ ")"
  | _ => "TODO"
  }
and debugEntry2 = ((k, v)) => theiaIRDebugPrint(k) ++ "->" ++ theiaIRDebugPrint(v)
and debugFreezer = ({ops, args, holePos}) => {
  "evalctx("
  ++ /* prettyList(List.map((s) => "\"" ++ s ++ "\"", ops)) ++ */ ", "
  ++ prettyList(List.map(theiaIRDebugPrint, args))
  ++ ", "
  ++ string_of_int(holePos)
  ++ ")";
};
...
let render_open_brack =
  <span style={ReactDOMRe.Style.make(~color="blue", ())}> {ReasonReact.string("[")} </span>;
let render_close_brack =
  <span style={ReactDOMRe.Style.make(~color="blue", ())}> {ReasonReact.string("]")} </span>;
...
and prettyKont2List = (~nestNum=0, ~parens=false, kn, fs) =>
  ...
  | [] => kn2Pretty(~parens, kn)
  | [f, ...fs] =>
    prettyFreeze(~nestNum, f, prettyKont2List(~nestNum=nestNum + 1, ~parens, kn, fs))
  }
and kn2PrettyList = xs =>
  xs
  |> List.map(s =>
       <div style={ReactDOMRe.Style.make(~fontSize="14px", ())}> {kn2Pretty(s)} </div>
     )
...
let handleClick = (~path, ~log, ~print=Theia, ~theiaIRTrace, dispatch, _event) => {
  dispatch(
    UpdateMachineState({k: List.map(kn2Pretty, theiaIRTrace)}),
    /* let callback = (json) =>
       switch (print) {
        ...
        <div
          ...
          <div
            ...
            /* {s->Belt.List.get(state.currentConfig - 1)->renderConfig} */
             {s->Belt.List.get(state.currentConfig)->renderConfig} </div>
          /* {s->Belt.List.get(state.currentConfig + 1)->renderConfig} */
